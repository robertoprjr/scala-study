...
trait FunSets extends FunSetsInterface {
   ...
   */
  def union(s: FunSet, t: FunSet): FunSet = (number => contains(s, number) || contains(t, number))

  /**
   * Returns the intersection of the two given sets,
   * the set of all elements that are both in `s` and `t`.
   */
  def intersect(s: FunSet, t: FunSet): FunSet = (number => contains(s, number) && contains(t, number))
   ...
   */
  def filter(s: FunSet, p: Int => Boolean): FunSet = (number => contains(s, number) && p(number))
