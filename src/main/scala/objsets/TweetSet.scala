...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new EmptyStackException
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  // Se passar pela condição, add o elemento... senão, mantem o existente
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if (p(elem))
  ...
  def mostRetweeted: Tweet = {
    ...
    def moreRetBtw(one: Tweet, two: Tweet): Tweet = if (one.retweets > two.retweets) one else two

    if (left.nonEmpty && right.nonEmpty)
      moreRetBtw(moreRetBtw(left.mostRetweeted, right.mostRetweeted), elem)
    else if (left.nonEmpty)
      moreRetBtw(left.mostRetweeted, elem)
    else if (right.nonEmpty)
      moreRetBtw(elem, right.mostRetweeted)
    else
      elem
  }
